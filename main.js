...
const { bloodmd } = require('./lib/system');
...
function bloodLog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [BLOOD-MD] ${new Date().toISOString()}: ${message}`);
}
...
bloodLog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { bloodLog(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                bloodLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            bloodLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            bloodLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                bloodLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { bloodLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                bloodLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    await bloodPair(number, mockRes);
                } catch (e) { bloodLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                bloodLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function bloodPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `blood_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            bloodLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                bloodLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            ...
            bloodLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }
        ...
        const bloodStore = createbloodStore();
...
const code = await conn.requestPairingCode(sanitizedNumber);
                ...
                bloodLog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            bloodLog(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                bloodLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });

        // Anti-delete
        conn.ev.on('messages.update', async (updates) => {
            await handleAntidelete(conn, updates, bloodStore);
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                await bloodmd(conn);
                bloodLog(`Connected: ${sanitizedNumber}`, 'success');
            ...
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) bloodLog(`Session logged out.`, 'error');
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363410830712574@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© BLOOD-MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:BLOOD-MD BOY\nORG:BLOOD-MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© BLOOD-MD' } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        } catch (e) { bloodLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
            ...
            } catch (e) { bloodLog(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        bloodLog(`BLOOD-MD Pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await bloodPair(req.query.number, res); });
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'BLOOD-MD is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 BLOOD-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        bloodLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { bloodLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        bloodLog('Auto-reconnect completed', 'success');
    } catch (e) { bloodLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
...
process.on('uncaughtException', (err) => {
    bloodLog(`Uncaught exception: ${err.message}`, 'error');
});
